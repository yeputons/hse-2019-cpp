...
* `static`-метод: как глобальная функция, static, external linkage, надо определить ровно один раз.
  ...
  Чем полезно: можно писать функции, которые имеют доступ к приватным полям и
  не засоряют глобальное пространство имён.
...
* Альтернативный способ написать, если мы не хотим добавлять в класс:
  ...
  ```
  Так симметричнее.
  И заодно работает с неявным преобразованием: `a + 10`, `10 + a`.
  Потому на что `this` неявное преобразование не распространяется.
* Чтобы можно было обращаться к приватным членам, можно объявить/определить как `friend` внутри класса:
  ```
  struct BigInt {
      friend BigInt operator+(const BigInt &a, const BigInt &b);
  };
  ```
* Дальше можно написать `+=` копипастом.
  Или сначала сделать `+=` методом (так нормально и симметричнее, чем `friend`),
  а `operator+` сделать не-`friend`:
  ```
  struct BigInt {
...
* Операторы сравнения: реализуем `==` и `<`, остальные выражаются.
  Если производительность неважна, то только `<` (иначе нельзя отсечь неравные строки по длине).
  * В C++20 появляется `operator<=>`, который позволяет выразить сразу все шесть операторов.
    До этого надо было писать свою `static int compare` и через неё всё делать.
