...
## Скрытие родительских методов [00:15]
* Бывает override выиртуального метода, но по умолчанию в нас везде hiding.
...
## Изменение видимости метода [00:05]
...
# Статические члены классов-константы [00:25]
...
## Старый способ для статических [00:15]
...
* Если у нас статическая константа целочисленного типа или enum,
  ...
  * Пример, когда не нужно: заиспользовали как размер массива.
  * Пример, когда нужно: взяли адрес.
  * Пример, когда нужно: инициализируется сложной функцией.
* Заумность: с C++17 можно сделать inline-переменную (отключит ODR),
  получим `static inline const`, но лучше не надо.

## Хороший способ [00:05]
...
# Виртуальное наследование [00:35]
## Основы [00:15]
