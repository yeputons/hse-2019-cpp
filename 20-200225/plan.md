# Шаблонный стэк с гарантиями исключений и превыделенной памятью [00:37]
## Напоминание про стэк и предвыделенную память [00:04]
...
* Надо сначала выделить кусок памяти из байт, а потом руками вызывать конструкторы и деструкторы.
  Для этого есть placement new и явный вызов деструктора: `new (ptr) T(....)` и `ptr->~T();`

## Обновление конструктора копирования [00:13]
```
...
## Обновление reserve и оператора присваивания [00:09]
```
void reserve(std::size_t newcap) {
    ...
    // for (len; len > 0; len--)  // Так нельзя, потому что меняется len, а его надо оставить,
...
# Move-семантика — использование [00:37]
## Проблемы [00:04]
...
4. Мы можем подсказать (обязательно в случае `unique_ptr`):
   ...
   ```
   Аналогично надо подсказывать в initializer list `Foo()`.
...
```
Если не поддерживает перемещение, будет копирование.
Что делать первым: `swap` или move, вопрос философский.
Потому что move очень удобно делать через swap.

## Параметры функций [00:07]
...
Но тут есть тонкости с вызовами деструкторов (с копированием у нас всегда создаётся временный объект,
а если два вида ссылок, то их не создаётся).
...
## Как использовать move-семантику [00:04]
...
3. Если функция что-то копирует, то __обычно__ принимает параметр по значению, а не по константной ссылке.
   Либо две перегрузки (это эффективнее): по константной обычной ссылке и по rvalue-ссылке.
4. Все мувы обычно noexcept.

## Move-семантика в стеке [00:04]
* `reserve` в стеке
