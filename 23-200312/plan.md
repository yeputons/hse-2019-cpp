...
* Последовательность символов.
  Похоже на `std::vector<char>`, но не гарантируется последовательная память и инвалидируется на любой неконстантный чих.
  Может храниться как rope/deque/что угодно.
* Есть `string == basic_string<char>` и `string == basic_string<wchar_t>`
* Можно даже свои traits для специальных символов
* Есть встроенные методы, которые работают с индексами вместо итераторов.
  * `a.substr(first, length = std::string::npos)`
  * `a.find(substr, startPos = 0) --> pos`
* Есть `c_str()`, возвращает сишную строку, удалять не надо.
  До C++11 была `data()`, там не гарантировался завершающий ноль.
* Есть `append`, `+=`, `+`...
