...
* Функции: TODO
  ...
  * параметры лучше не умными указателями, а по значению. Ссылками, если очень надо.
  * `const unique_ptr<>&`/`const shared_ptr<>&` бесполезны абсолютно, ссылка или чистый указатель (если можно занулить).
  * `unique_ptr<>&`/`shared_ptr<>&` можно принимать только если вы их собираетесь для вызывающего менять.
  * `unique_ptr<T>` принимаем только в вершинах дерева. Если было бы разумно `T` хранить по значению, то принимаем `T`.
  * Возвращаемое значение аналогично.
  * Не форсируйте умные указатели
