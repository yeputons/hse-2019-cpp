...
* Функции:
  ...
  * Возвращаемые значения лучше всегда передавать по значению, иначе можно огрести с временем жизни.
    * `T&` возвращается в `operator[]`.
    * `const T&` возвращается в геттерах полей.
